...
<div class="order-details bg-platinum dashboard-space">
    <div class="header-dual-part">
        <a href="{{ url('/orders') }}" class="text-light-black d-inline-flex align-items-center mgb-20">
            <svg class="me-2" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M9.57086 5.18001C9.76086 5.18001 9.95086 5.25 10.1009 5.4C10.3909 5.69 10.3909 6.17 10.1009 6.46L4.56086 12L10.1009 17.54C10.3909 17.83 10.3909 18.31 10.1009 18.6C9.81086 18.89 9.33086 18.89 9.04086 18.6L2.97086 12.53C2.68086 12.24 2.68086 11.76 2.97086 11.47L9.04086 5.4C9.19086 5.25 9.38086 5.18 9.57086 5.18001Z"
                    fill="#2F415E" />
                <path
        ...
        <a href="{{ route('orders.create')}}"
            class="button button-primary h-40 d-inline-flex align-items-center py-2">Add new order</a>
    </div>
    @php $order_id = request()->route('id') @endphp


    {{-- header --}}
    <order-header :order="{{ $order }}" :share-url="'{{ url('/public-order/'. base64_encode($order_id) ) }}'"
        :diff_in_days="{{ $diff_in_days }}" :diff_in_hours="{{ $diff_in_hours }}"></order-header>
    ...
    <div class="order-details-box-main row">
        <div class="order-details__left col-md-6">
            ...
            {{-- Property Info --}}
            <property-info :appraisal_types="{{ $appraisal_types }}" :order="{{ $order }}"
                :order-id="'{{ $order_id }}'"></property-info>
            {{-- Borrower --}}
        ...
        <div class="order-details__right col-md-6">
            {{-- Appraisal Details --}}
            <appraisal-details :order="{{ $order }}" :property-types="{{ $property_types }}"
                :appraisers="{{ $appraisers }}" :loan-types="{{ $loan_types }}" :order-id="'{{ $order_id }}'">
            </appraisal-details>
            {{-- Provided Services --}}
            <provided-services :order="{{ $order }}" :appraisal-types="{{ $appraisal_types }}"
                :order-id="'{{ $order_id }}'"></provided-services>
            {{-- Client --}}
            <client-info :order="{{ $order }}" :all-amc="{{ $all_amc }}" :all-lender="{{ $all_lender }}"
                :order-id="'{{ $order_id }}'"></client-info>
            {{-- Call log --}}
    ...
    <div class="mgt-32">
        <files :order-files="{{ json_encode($order_files) }}" :order="{{ $order }}"
            :order-file-types="{{ json_encode($order_file_types) }}"></files>
    </div>
    <div class="mgt-32">
